...
async function convertIfcToGlb(ifcFilePath, outputFilePath) {
  ...
  ifcapi.StreamAllMeshes(modelID, (mesh, index) => {
    ...
    for (let i = 0; i < placedGeometries.size(); i++) {
      ...
      //   const [x, y, z] = applyTransformation(placedGeometry.flatTransformation, verts[j], verts[j + 1], verts[j + 2]);
      //   transformedVerts.push(x, y, z);

      //   const [nx, ny, nz] = applyTransformation(placedGeometry.flatTransformation, normals[j], normals[j + 1], normals[j + 2]);
...
// Run the conversion
convertIfcToGlb('example.ifc', 'example.glb')
